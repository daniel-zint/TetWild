...
void saveFinalTetmesh(
    const std::string& output_volume,
    const std::string& output_surface,
...
int main(int argc, char* argv[])
{
    ...
    app.add_option(
        "output,--output",
        output_volume,
        "Output tetmesh OUTPUT in .msh or .mesh format. (string, optional, default: "
        "input_file+postfix+'.msh')");
    app.add_option(
        "--postfix",
        args.postfix,
        "Postfix P for output files. (string, optional, default: '_')");
    auto absolute = app.add_option(
        "-a,--ideal-absolute-edge-length",
        args.initial_edge_len_abs,
        "Absolute edge length (not scaled by bbox). -a and -l cannot both be given as arguments.");
    auto relative = app.add_option(
        "-l,--ideal-edge-length",
        args.initial_edge_len_rel,
    ...
    app.add_option(
        "--stage",
        args.stage,
        "Run pipeline in stage STAGE. (integer, optional, default: 1)");
    app.add_option(
        "--filter-energy",
        args.filter_energy_thres,
        "Stop mesh improvement when the maximum energy is smaller than ENERGY. (double, optional, "
        "default: 10)");
    app.add_option(
        "--max-pass",
        args.max_num_passes,
        "Do PASS mesh improvement passes in maximum. (integer, optional, default: 80)");
    app.add_option(
        "--targeted-num-v",
        args.target_num_vertices,
        "Output tetmesh that contains TV vertices. (integer, optional, tolerance: 5%)");
    app.add_option(
        "--bg-mesh",
        args.background_mesh,
        "Background tetmesh BGMESH in .msh format for applying sizing field. (string, optional)");
    app.add_option("--log", log_filename, "Log info to given file.");
    app.add_option("--level", log_level, "Log level (0 = most verbose, 6 = off).");
    app.add_option(
        "--save-mid-result",
        args.save_mid_result,
        "Get result without winding number: --save-mid-result 2");

    app.add_option(
        "--bbox-dis",
        args.bbox_dis,
        "Relative bbox distance to the input (optional, default: 0.05)");

    app.add_flag(
        "--no-voxel",
        args.not_use_voxel_stuffing,
        "Use voxel stuffing before BSP subdivision.");
    app.add_flag(
        "--is-laplacian",
        args.smooth_open_boundary,
        "Do Laplacian smoothing for the surface of output on the holes of input (optional)");
    ...
    if (slz_file != "") {
        gtet_new_slz(VI, FI, slz_file, {{true, false, true, true}}, VO, TO, AO, args);
    } else {
