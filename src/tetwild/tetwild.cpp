    ...
    const State& state)
{
    ...
    std::array<double, 6> cmp_d_angles = {
        {6 / 180.0 * M_PI,
         12 / 180.0 * M_PI,
         18 / 180.0 * M_PI,
         162 / 180.0 * M_PI,
    ...
    std::vector<std::array<int, 3>>& m_faces)
{
    ...
    addRecord(
        MeshRecord(
            MeshRecord::OpType::OP_INIT,
            0,
            geo_sf_mesh.vertices.nb(),
            geo_sf_mesh.facets.nb()),
        args,
        state);
    ...
    addRecord(
        MeshRecord(
            MeshRecord::OpType::OP_PREPROCESSING,
            tmp_time,
            m_vertices.size(),
            m_faces.size()),
        args,
    ...
    std::vector<std::vector<int>>& raw_conn_e4v)
{
    ...
    addRecord(
        MeshRecord(
            MeshRecord::OpType::OP_DELAUNEY_TETRA,
            tmp_time,
            bsp_vertices.size(),
            bsp_nodes.size()),
        args,
...
double tetwild_stage_one_mc(const Args& args, const State& state, MeshConformer& MC)
{
    igl::Timer igl_timer;
    igl_timer.start();
    ...
    addRecord(
        MeshRecord(
            MeshRecord::OpType::OP_DIVFACE_MATCH,
            tmp_time,
            MC.bsp_vertices.size(),
            MC.bsp_nodes.size()),
        args,
        state);
    logger().info("time = {}s", tmp_time);
...
double tetwild_stage_one_bsp(const Args& args, const State& state, MeshConformer& MC)
{
    igl::Timer igl_timer;
    igl_timer.start();
    ...
    std::vector<std::array<int, 4>>& is_surface_facet)
{
    ...
    addRecord(
        MeshRecord(
            MeshRecord::OpType::OP_SIMPLE_TETRA,
            tmp_time,
            tet_vertices.size(),
            tet_indices.size()),
        args,
    ...
    std::vector<std::array<int, 4>>& is_surface_facet)
{
    ...
    std::vector<Point_3> m_vertices;
    std::vector<std::array<int, 3>> m_faces;
    sum_time += tetwild_stage_one_preprocess(
        VI,
        FI,
        args,
        state,
    ...
    std::vector<int> raw_e_tags;
    std::vector<std::vector<int>> raw_conn_e4v;
    sum_time += tetwild_stage_one_delaunay(
        args,
        state,
        geo_sf_mesh,
        m_vertices,
    ...
    sum_time += tetwild_stage_one_tetra(
        args,
        state,
        MC,
        m_f_tags,
