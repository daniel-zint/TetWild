...
void MeshRefinement::round()
{
    ...
    for (int i = 0; i < tet_vertices.size(); i++) {
        ...
        tet_vertices[i].pos =
            Point_3(tet_vertices[i].posf[0], tet_vertices[i].posf[1], tet_vertices[i].posf[2]);

        for (auto it = tet_vertices[i].conn_tets.begin(); it != tet_vertices[i].conn_tets.end();
             it++) {
            CGAL::Orientation ori;
            ori = CGAL::orientation(
                tet_vertices[tets[*it][0]].pos,
                tet_vertices[tets[*it][1]].pos,
                tet_vertices[tets[*it][2]].pos,
                tet_vertices[tets[*it][3]].pos);
    ...
    int scalar_update)
{
    ...
    if (args.save_mid_result == 1) outputMidResult(false, 1);
        ...
        if (max_e > 100) {
            ...
            for (int i = 0; i < tet_qualities.size(); i++) {
                ...
                if (tet_qualities[i].slim_energy > max_e * 0.9) {
                    ...
                    for (int j = 0; j < 3; j++) {
                        l[j * 2] = CGAL::squared_distance(
                            tet_vertices[tets[i][0]].posf,
                            tet_vertices[tets[i][j + 1]].posf);
                        l[j * 2 + 1] = CGAL::squared_distance(
                            tet_vertices[tets[i][j + 1]].posf,
    ...
    if (args.save_mid_result == 2) outputMidResult(true, 2); // mark in/out
...
void MeshRefinement::markInOut(std::vector<bool>& tmp_t_is_removed)
{
    ...
    for (int i = 0; i < tets.size(); i++) {
        ...
        vs.reserve(4);
        for (int j = 0; j < 4; j++) vs.push_back(tet_vertices[tets[i][j]].posf);
        Point_3f p = CGAL::centroid(vs.begin(), vs.end(), CGAL::Dimension_tag<0>());
        for (int j = 0; j < 3; j++) C(cnt, j) = p[j];
        cnt++;
    }
...
void MeshRefinement::applySizingField(
    EdgeSplitter& splitter,
    EdgeCollapser& collapser,
    EdgeRemover& edge_remover,
    VertexSmoother& smoother)
{
    ...
    if (V_in.rows() == 0 || T_in.rows() == 0 || values.rows() == 0) return;
    ...
    for (int i = 0; i < V_in.rows() / 3; i++) {
        ...
        for (int j = 0; j < 3; j++) p[j] = V_in(i * 3 + j);
    }
    bg_mesh.cells.clear();
    bg_mesh.cells.create_tets((int)T_in.rows() / 4);
    for (int i = 0; i < T_in.rows() / 4; i++) {
        for (int j = 0; j < 4; j++) bg_mesh.cells.set_vertex(i, j, T_in(i * 4 + j));
    }
    ...
    for (int i = 0; i < tet_vertices.size(); i++) {
        ...
        if (bg_t_id == GEO::MeshCellsAABB::NO_TET) continue;
        ...
        for (int j = 0; j < 4; j++) {
            ...
            double weight = std::sqrt(
                CGAL::squared_distance(tet_vertices[i].posf, pln) /
                CGAL::squared_distance(vs[(j + 3) % 4], pln));
            weights[j] = weight;
            value += weight * values(T_in(bg_t_id * 4 + (j + 3) % 4));
        }

        tet_vertices[i].adaptive_scale =
            value / state.initial_edge_len; // we allow .adaptive_scale > 1
    }
...
void MeshRefinement::check()
{
    for (int i = 0; i < tet_vertices.size(); i++) {
        ...
        for (auto it = tet_vertices[i].conn_tets.begin(); it != tet_vertices[i].conn_tets.end();
             it++)
            if (t_is_removed[*it]) logger().debug("v {} should have conn_tet t{}", i, *it);
    ...
    for (int i = 0; i < tets.size(); i++) {
        ...
        CGAL::Orientation ori = CGAL::orientation(
            tet_vertices[tets[i][0]].pos,
        ...
        for (int j = 0; j < 4; j++) {
            ...
            for (auto it = tet_vertices[tets[i][j]].conn_tets.begin();
                 it != tet_vertices[tets[i][j]].conn_tets.end();
                 it++) {
                if (*it == i) {
                    is_found = true;
                }
                if (t_is_removed[*it]) logger().debug("tet {} is removed!", *it);
            }
            if (!is_found) {
    ...
    for (int i = 0; i < tet_faces.size(); i++) {
        ...
        setIntersection(
            tet_vertices[tet_faces[i][0]].conn_tets,
            tet_vertices[tet_faces[i][1]].conn_tets,
            tmp);
        setIntersection(tet_vertices[tet_faces[i][2]].conn_tets, tmp, tmp);

        if (tmp.size() != 1 && tmp.size() != 2) logger().debug("{}", tmp.size());
    }
}
...
void MeshRefinement::outputMidResult(bool is_with_bbox, double id)
{
    ...
    for (int i = 0; i < tets.size(); i++) {
        if (tmp_t_is_removed[i]) continue;
        for (int j = 0; j < 4; j++) v_ids.push_back(tets[i][j]);
        t_cnt++;
    }
    std::sort(v_ids.begin(), v_ids.end());
    v_ids.erase(std::unique(v_ids.begin(), v_ids.end()), v_ids.end());
    std::unordered_map<int, int> map_ids;
    for (int i = 0; i < v_ids.size(); i++) map_ids[v_ids[i]] = i;

    PyMesh::MshSaver mSaver(
        state.working_dir + state.postfix + "_mid" + std::to_string(id) + ".msh",
        true);
    Eigen::VectorXd oV(v_ids.size() * 3);
    Eigen::VectorXi oT(t_cnt * 4);
    for (int i = 0; i < v_ids.size(); i++) {
        for (int j = 0; j < 3; j++) oV(i * 3 + j) = tet_vertices[v_ids[i]].posf[j];
    }
    //    int cnt = 0;
    cnt = 0;
    for (int i = 0; i < tets.size(); i++) {
        if (tmp_t_is_removed[i]) continue;
        for (int j = 0; j < 4; j++) oT(cnt * 4 + j) = map_ids[tets[i][j]];
        cnt++;
    }
    ...
    if (is_with_bbox) mSaver.save_elem_scalar_field("in_out", in_out);
    ...
    for (int i = 0; i < tets.size(); i++) {
        ...
        for (int j = 0; j < 4; j++) {
            if (is_surface_fs[i][j] != state.NOT_SURFACE && is_surface_fs[i][j] > 0) { // outside
                ...
                if (CGAL::orientation(
                        tet_vertices[v_ids[0]].pos,
                        tet_vertices[v_ids[1]].pos,
                        tet_vertices[v_ids[2]].pos,
                        tet_vertices[tets[i][j]].pos) != CGAL::POSITIVE) {
                    int tmp = v_ids[0];
                    v_ids[0] = v_ids[2];
                    v_ids[2] = tmp;
                }
...
bool MeshRefinement::deserialization(
    const Eigen::MatrixXd& V_in,
    const Eigen::MatrixXi& F_in,
    const std::string& slz_file)
{
    ...
    // process sf_file
    if (!getSurfaceMesh(V_in, F_in, geo_sf_mesh)) return false;
...
void MeshRefinement::serialization(const std::string& slz_file)
{
    ...
    for (int i = 0; i < tets.size(); i++) {
        ...
        for (int j = 0; j < 4; j++) slz_tets.back()[j] = new_v_ids[tets[i][j]];
    }
