...
void LocalOperations::comformalAMIPSHessian_new(const double* T, double* result_0)
{
    ...
    result_0[0] = helper_56 * (helper_57 * helper_64 * helper_65 - pow(helper_64, 2) * helper_83 +
    ...
    result_0[8] = helper_56 * (-helper_108 * helper_109 * helper_65 -
...
void LocalOperations::check()
{
    ...
    for (int i = 0; i < tet_vertices.size(); i++) {
        ...
        for (auto it = tet_vertices[i].conn_tets.begin(); it != tet_vertices[i].conn_tets.end();
             it++) {
            if (t_is_removed[*it]) logger().debug("t {} is removed!", *it);
...
void LocalOperations::outputInfo(int op_type, double time, bool is_log)
{
    ...
    for (int i = 0; i < tets.size(); i++) {
        if (!t_is_removed[i]) calTetQuality_AD(tets[i], tet_qualities[i]);
    }
    ...
    for (int i = 0; i < tet_qualities.size(); i++) {
        ...
        cnt++;
        if (tet_qualities[i].min_d_angle < min) min = tet_qualities[i].min_d_angle;
        if (tet_qualities[i].max_d_angle > max) max = tet_qualities[i].max_d_angle;
        ...
        for (int j = 0; j < 3; j++) {
            if (tet_qualities[i].min_d_angle < cmp_d_angles[j]) cmp_cnt[j]++;
        }
        for (int j = 0; j < 3; j++) {
            if (tet_qualities[i].max_d_angle > cmp_d_angles[j + 3]) cmp_cnt[j + 3]++;
        }
    }

    logger().debug(
        "min_d_angle = {}, max_d_angle = {}, max_slim_energy = {}",
        min,
        max,
        max_slim_energy);
    logger().debug(
        "avg_min_d_angle = {}, avg_max_d_angle = {}, avg_slim_energy = {}",
        min_avg / cnt,
        max_avg / cnt,
        avg_slim_energy / cnt);
    logger().debug(
    ...
    if (is_log) {
        addRecord(
            MeshRecord(
                op_type,
                time,
                std::count(v_is_removed.begin(), v_is_removed.end(), false),
                cnt,
                min,
                min_avg / cnt,
                max,
...
bool LocalOperations::isTetFlip(const std::array<int, 4>& t)
{
    ...
    if (is_rounded)
        ...
        ori = CGAL::orientation(
            tet_vertices[t[0]].pos,
            tet_vertices[t[1]].pos,
            tet_vertices[t[2]].pos,
            tet_vertices[t[3]].pos);

    if (ori != CGAL::POSITIVE) return true;
    return false;
}
    ...
    bool all_measure)
{
    ...
    for (int i = 0; i < new_tets.size(); i++) {
        CGAL::Orientation ori = CGAL::orientation(
            tet_vertices[new_tets[i][0]].posf,
...
void LocalOperations::calTetQuality_AD(const std::array<int, 4>& tet, TetQuality& t_quality)
{
    ...
    for (int i = 0; i < 4; i++) {
        ...
        // re-scale
        std::array<double, 3> tmp_nv = {
            {CGAL::abs(nv[i][0]), CGAL::abs(nv[i][1]), CGAL::abs(nv[i][2])}};
...
bool LocalOperations::isPointOutEnvelop(const Point_3f& p)
{
    ...
    if (geo_sf_tree.squared_distance(geo_p) > state.eps_2) return true;
...
{
#if CHECK_ENVELOP
    if (tri.is_degenerate()) return false;
...
bool LocalOperations::isBoundarySlide(int v1_id, int v2_id, Point_3f& old_pf)
{
    ...
    for (int t_id : tet_vertices[v1_id].conn_tets) {
        for (int j = 0; j < 4; j++)
            if (tets[t_id][j] != v1_id && tets[t_id][j] != v2_id &&
                tet_vertices[tets[t_id][j]].is_on_boundary)
                n_v_ids.insert(tets[t_id][j]);
    }
    if (n_v_ids.size() == 0) return false;
    ...
    for (int v_id : n_v_ids) {
        if (!isEdgeOnBoundary(v1_id, v_id)) continue;
        // sample the edge (v1, v) and push the sampling points into vector
        GEO::vec3 p1(
            tet_vertices[v1_id].posf[0],
            tet_vertices[v1_id].posf[1],
            tet_vertices[v1_id].posf[2]);
        GEO::vec3 p2(
            tet_vertices[v_id].posf[0],
            tet_vertices[v_id].posf[1],
    ...
    if (v2_id >= 0 && tet_vertices[v2_id].is_on_boundary) {
        ...
        for (int v_id : n12_v_ids) {
            if (!isEdgeOnBoundary(v1_id, v_id) || !isEdgeOnBoundary(v2_id, v_id)) continue;
            if (!is_12_on_boundary) {
                ...
                for (int k = 1; k <= n - 1; k++)
                    b_points.push_back(
                        p1 * ((double)k / (double)n) + p2 * ((double)(n - k) / (double)n));
                b_points.push_back(p2);
            } else {
    ...
    breakdown_timing0[id_sampling] += igl_timer0.getElapsedTime();
#endif
    if (b_points.size() == 0) return false;
...
void LocalOperations::outputSurfaceColormap(
    ...
    double old_eps)
{
    ...
    PyMesh::MshSaver mshSaver(
        state.working_dir + args.postfix + "_sf" + std::to_string(mid_id++) + ".msh");
