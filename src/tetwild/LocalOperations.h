...
public:
    ...
    LocalOperations(
        ...
        const GEO::MeshFacetsAABBWithEps& geo_tree,
        const GEO::MeshFacetsAABBWithEps& b_t,
        const Args& ar,
        State& st)
        : tet_vertices(t_vs)
        , tets(ts)
    ...
    int mid_id = 0;
    void
    outputSurfaceColormap(const Eigen::MatrixXd& V_in, const Eigen::MatrixXi& F_in, double old_eps);
