...
void checkBoundary(const Eigen::MatrixXd& V, const Eigen::MatrixXi& F, const State& state)
{
    ...
    for (int i = 0; i < V.rows(); i++) {
        for (int j = 0; j < 3; j++) oV(i * 3 + j) = V(i, j);
    }
    oF.resize(F.rows() * 3);
    for (int i = 0; i < F.rows(); i++) {
        for (int j = 0; j < 3; j++) oF(i * 3 + j) = F(i, j);
    }
    ...
    if (oV.rows() > 0) {
        ...
        for (int i = 0; i < bF.rows(); i++) bF(i) = 0;
        Eigen::VectorXd bV(V.rows());
        for (int i = 0; i < bV.rows(); i++) bV(i) = 0;

        std::vector<std::vector<int>> conn_f4v(V.rows(), std::vector<int>());
        for (int i = 0; i < F.rows(); i++) {
            for (int j = 0; j < 3; j++) conn_f4v[F(i, j)].push_back(i);
        }
        ...
        for (int i = 0; i < F.rows(); i++) {
            for (int j = 0; j < 3; j++) {
                ...
                std::set_intersection(
                    conn_f4v[F(i, j)].begin(),
                    conn_f4v[F(i, j)].end(),
                    conn_f4v[F(i, (j + 1) % 3)].begin(),
                    conn_f4v[F(i, (j + 1) % 3)].end(),
...
bool Preprocess::init(
    const Eigen::MatrixXd& V_tmp,
    const Eigen::MatrixXi& F_tmp,
    GEO::Mesh& geo_b_mesh,
    ...
    const Args& args)
{
    ...
    for (int i = 0; i < V_in.rows(); i++) {
        ...
        for (int j = 0; j < 3; j++) p[j] = V_in(i, j);
    }
    geo_sf_mesh.facets.clear();
    geo_sf_mesh.facets.create_triangles((int)F_in.rows());
    for (int i = 0; i < F_in.rows(); i++) {
        for (int j = 0; j < 3; j++) geo_sf_mesh.facets.set_vertex(i, j, F_in(i, j));
    }
...
void Preprocess::getBoundaryMesh(GEO::Mesh& b_mesh)
{
    ...
    for (int i = 0; i < F_sf.rows(); i++) {
        for (int j = 0; j < 3; j++) {
            std::vector<int> tmp;
            std::set_intersection(
                conn_f4v[F_sf(i, j)].begin(),
                conn_f4v[F_sf(i, j)].end(),
                conn_f4v[F_sf(i, (j + 1) % 3)].begin(),
                conn_f4v[F_sf(i, (j + 1) % 3)].end(),
                std::back_inserter(tmp));
            if (tmp.size() == 1)
    ...
    for (int v_id : v_ids) {
        ...
        for (int j = 0; j < 3; j++) p[j] = V_sf(v_id, j);
    }
    b_mesh.facets.clear();
    ...
    const Args& args)
{
    ...
    for (int i = 0; i < F_in.rows(); i++) {
        for (int j = 0; j < 3; j++) conn_fs[F_in(i, j)].insert(i);
    }
    ...
    for (int i = 0; i < F_in.rows(); i++) {
        ...
        for (int j = 0; j < 3; j++) F_out(cnt, j) = new_v_ids[F_in(i, j)];
        cnt++;
    }
...
void Preprocess::swap(const GEO::Mesh& geo_mesh, const GEO::MeshFacetsAABBWithEps& face_aabb_tree)
{
    ...
    for (int i = 0; i < F_in.rows(); i++) {
        ...
        for (int j = 0; j < 3; j++) {
            ...
            if (n12_f_ids[1] == i) n12_f_ids = {{n12_f_ids[1], n12_f_ids[0]}};
            int v3_id = -1;
            for (int k = 0; k < 3; k++)
            ...
            for (int f = 0; f < 2; f++) {
                ...
                for (int k = 0; k < 3; k++) {
                    vs[k] = GEO::vec3(
                        V_in(F_in(n12_f_ids[f], k), 0),
                        V_in(F_in(n12_f_ids[f], k), 1),
            ...
            if (std::min(cos_a_new, cos_a1_new) <= std::min(cos_a, cos_a1)) continue;

            // non flipping
            Eigen::RowVectorXi f1_old = F_in.row(n12_f_ids[0]);
            Eigen::RowVectorXi f2_old = F_in.row(n12_f_ids[1]);
            for (int k = 0; k < 3; k++) {
                if (F_in(n12_f_ids[0], k) == v2_id) F_in(n12_f_ids[0], k) = v3_id;
                if (F_in(n12_f_ids[1], k) == v1_id) F_in(n12_f_ids[1], k) = v_id;
            }
...
{
    return GEO::Geom::cos_angle(
        GEO::vec3(V_in(v1_id, 0), V_in(v1_id, 1), V_in(v1_id, 2)) -
...
void Preprocess::simplify(
    const GEO::Mesh& geo_mesh,
    const GEO::MeshFacetsAABBWithEps& face_aabb_tree)
{
    int cnt = 0;
    ...
    while (!sm_queue.empty()) {
        ...
        if (!isEdgeValid(v_ids, old_weight)) continue;
        ...
        if (!removeAnEdge(v_ids[0], v_ids[1], geo_mesh, face_aabb_tree)) {
            ...
            cnt++;
            if (cnt % 1000 == 0) logger().debug("1000 vertices removed");
        }
    }
    logger().debug("{}", cnt);
    logger().debug("{}", c);

    if (cnt > 0) postProcess(geo_mesh, face_aabb_tree);
}

void Preprocess::postProcess(
    const GEO::Mesh& geo_mesh,
    const GEO::MeshFacetsAABBWithEps& face_aabb_tree)
{
...
bool Preprocess::removeAnEdge(
    int v1_id,
    int v2_id,
    const GEO::Mesh& geo_mesh,
    const GEO::MeshFacetsAABBWithEps& face_aabb_tree)
{
    if (!isOneRingClean(v1_id) || !isOneRingClean(v2_id)) return false;
    ...
    if (!isEuclideanValid(v1_id, v2_id)) return false;
    ...
    for (int f_id : new_f_ids) {
        ...
        for (int j = 0; j < 3; j++) {
            vs[j] =
                GEO::vec3(V_in(F_in(f_id, j), 0), V_in(F_in(f_id, j), 1), V_in(F_in(f_id, j), 2));
        }
        ...
        GEO::vec3 new_nv = GEO::Geom::triangle_normal(vs[0], vs[1], vs[2]);
        if (GEO::dot(old_nv, new_nv) < 0) return false;
        //        }
    }
    ...
    for (int j = 0; j < 3; j++) V_in(v1_id, j) = nearest_p[j];
    ...
    for (int f_id : new_f_ids) {
        for (int j = 0; j < 3; j++) {
            if (F_in(f_id, j) != v1_id && F_in(f_id, j) != v2_id) n_v_ids.insert(F_in(f_id, j));
        }
    }
...
bool Preprocess::isEdgeValid(const std::array<int, 2>& v_ids)
{
    ...
    if (!isHaveCommonEle(conn_fs[v_ids[0]], conn_fs[v_ids[1]])) return false;
...
bool Preprocess::isOutEnvelop(
    const std::unordered_set<int>& new_f_ids,
    const GEO::Mesh& geo_sf_mesh,
    const GEO::MeshFacetsAABBWithEps& geo_face_tree)
{
...
{
    if (geo_face_tree.squared_distance(GEO::vec3(V_in(v_id, 0), V_in(v_id, 1), V_in(v_id, 2))) >
        state.eps_2)
        return true;
    return false;
}
...
bool Preprocess::isEuclideanValid(int v1_id, int v2_id)
{
    ...
    for (int I = 0; I < 2; I++) {
        ...
        for (int f_id : conn_fs[v_id]) {
            if (F_in(f_id, 0) != F_in(f_id, 1) && F_in(f_id, 1) != F_in(f_id, 2) &&
                F_in(f_id, 0) != F_in(f_id, 2)) {
    ...
    if (ec0 != ec1) return false;
    return true;
}

void Preprocess::outputSurfaceColormap(
    const GEO::MeshFacetsAABBWithEps& geo_face_tree,
    const GEO::Mesh& geo_sf_mesh)
{
    ...
    for (int f_id = 0; f_id < geo_sf_mesh.facets.nb(); f_id++) {
        ...
        GEO::index_t prev_facet =
            geo_face_tree.nearest_facet(current_point, nearest_point, sq_dist);
        ...
        if (f_id == 1681) {
            for (const GEO::vec3& p : ps) {
                logger().debug(
                    ...
                    p[2],
                    fs[cnt],
                    geo_face_tree.squared_distance(p));
                cnt++;
            }
        }
        ...
        if (eps_dis(f_id) > 1) {
            ...
            for (int j = 0; j < vf.size(); j++) {
                ...
                for (int k = 0; k < 3; k++) {
                    logger().debug(
                        "{}: {} {} {}",
                        v_ids[k],
                        geo_sf_mesh.vertices.point(v_ids[k])[0],
                        geo_sf_mesh.vertices.point(v_ids[k])[1],
                        geo_sf_mesh.vertices.point(v_ids[k])[2]);
                }
            }
            ...
            for (int i = 0; i < geo_sf_mesh.facets.nb(); i++) {
                double s_dis = GEO::Geom::point_triangle_squared_distance(
                    ps[10],
                    geo_sf_mesh.vertices.point(geo_sf_mesh.facets.vertex(i, 0)),
                    geo_sf_mesh.vertices.point(geo_sf_mesh.facets.vertex(i, 1)),
                    geo_sf_mesh.vertices.point(geo_sf_mesh.facets.vertex(i, 2)),
                    nearest_p,
                    _1,
                    _2,
                    _3);
            ...
            for (int i = 0; i < geo_sf_mesh.facets.nb(); i++) {
                if (diss[i] == min_dis) logger().debug("vf_id = {}", i);
            }
            ...
            logger().debug(
                ...
                GEO::Geom::point_triangle_squared_distance(
                    ps[10],
                    geo_sf_mesh.vertices.point(geo_sf_mesh.facets.vertex(1681, 0)),
                    geo_sf_mesh.vertices.point(geo_sf_mesh.facets.vertex(1681, 1)),
                    geo_sf_mesh.vertices.point(geo_sf_mesh.facets.vertex(1681, 2)),
                    nearest_p,
                    _1,
                    _2,
                    _3));
    ...
    for (int i = 0; i < V_in.rows(); i++) {
        for (int j = 0; j < 3; j++) V_vec(i * 3 + j) = V_in(i, j);
    }
    for (int i = 0; i < F_in.rows(); i++) {
        for (int j = 0; j < 3; j++) F_vec(i * 3 + j) = F_in(i, j);
    }
